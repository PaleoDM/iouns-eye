...
- Grow the codex organically as a byproduct of play (via automated extraction from recaps), not as a separate documentation chore
- Replace the Adobe Illustrator calendar with an interactive, data-driven component
- Make the world maps clickable and linked to codex entries
...
- Codex entry files must be plain markdown readable/writable by Claude Code
...
4. Creates or updates codex entry files in the Ioun's Eye repo
5. Updates INDEX.md

### Session Prep with Codex Context
...
│       ├── src/content/ ← CODEX ENTRIES (source of truth)     │
...
4. **INDEX.md as Claude's Entry Point** — a flat, auto-generated file listing every codex entry with its slug, type, name, and one-line description. Claude reads this single file to know what exists, then reads specific entries as needed. Regenerated by a build script.
...
**Objective**: Extract all campaign-agnostic world knowledge from the World Building folder into codex entries and structured JSON data files.
...
4. Create deity codex entries in `src/content/deities/` — one `.md` file per deity with richer prose descriptions, worship traditions, and role in the world
5. Create lore codex entries for foundational topics:
...
- [ ] Every prime deity has both a JSON record and a codex entry
...
**Objective**: Extract all NPCs, locations, factions, events, items, and plot details from the five completed campaigns into codex entries.
...
**Objective**: Extract all entities from Ishetar 2.0 (~30 sessions) and Rifthaven Online (~43 sessions) into codex entries.
...
- [ ] Every named NPC from all session recaps has a codex entry
...
**Objective**: Build a data-driven, interactive circular calendar that faithfully reproduces the Illustrator design and links to codex events.
...
3. **Add interactivity**:
   ...
   - Click on a day: show events from the codex that occurred on this date
...
4. **Create calendar page** (`src/pages/calendar.astro`):
   ...
   - Link from calendar events to their codex entry pages
...
- [ ] Clicking a date shows events from the codex for that date
...
**Objective**: Make the world maps interactive with clickable regions linking to codex location entries.
...
2. **Define clickable regions**:
   - Create `src/data/map-regions.json` mapping region IDs to:
     ...
     - Location slug (codex entry to link to)
...
3. **Build map component** (`src/components/map/WorldMap.tsx`):
   ...
   - Click: navigate to location codex entry
...
- [ ] Clickable regions correspond to codex location entries
...
1. **Upgrade `/write-recap`**:
   - After writing the recap, add an extraction step
   - Scan the new recap text for named entities (NPCs, locations, factions, items, events)
   - For each entity:
     - Check INDEX.md — does an entry already exist?
     - If yes: update the existing entry with new information from this session
     - If no: create a new entry with information from the recap
   - Update `related` fields for entities that interacted in this session
   - Regenerate INDEX.md
   - Report: "Created X new entries, updated Y existing entries"

2. **Upgrade `/prep-session`**:
   - Before generating the prep, add a codex lookup step
   - Read INDEX.md
   - Read the most recent session prep and recap (existing behavior)
   - Identify entities likely relevant to the upcoming session:
     - NPCs mentioned in recent sessions
     - Locations the party is at or heading to
     - Active factions and their current goals
     - Open plot threads from events
   - Read full codex entries for identified entities
   - Pass this context into the prep generation prompt
   - Result: preps reference established lore consistently and don't contradict previous sessions

3. **Test both skills end-to-end**:
   - Run `/write-recap` on a recent session and verify extraction
   - Run `/prep-session` for an upcoming session and verify codex context enriches the output

**Files Created/Modified**:
- `../.claude/skills/write-recap/SKILL.md` — Updated with extraction step
- `../.claude/skills/prep-session/SKILL.md` — Updated with codex lookup step

**Success Criteria**:
- [ ] `/write-recap` produces the recap AND extracts entities to codex
- [ ] New NPCs from a recap appear as codex entries after the skill runs
- [ ] Existing NPCs are updated (not duplicated) when they appear in a new recap
- [ ] `/prep-session` reads codex entries and produces richer, more contextual preps
- [ ] Prep output references established lore correctly (names, dates, relationships match codex)
- [ ] Neither skill breaks existing functionality — recaps and preps still work as before

**Known Risks**:
- Entity extraction from prose is imprecise — may create entries for minor/unnamed characters
- The extraction step adds significant time to the recap skill — keep it focused on named entities only
- Prep skill context window: reading too many codex entries could overwhelm the prompt — need to be selective
- Skills reference paths in the other repo — path configuration must be robust
...
1. **Repo reorganization** (parent TTRPG repo):
   - Assess current structure: `Art Commissions/`, `Assets & Resources/`, `Campaigns/`, `VTT Product/`, `World Building/`, `iouns-eye/`
   - Archive completed campaigns that have been fully extracted (move to `Campaigns/Archive/` or similar)
   - CONSTRAINT: Do not change root path or VTT Product path
   - Clean up `World Building/` — foundational data is now in iouns-eye, but keep Illustrator source files

2. **Migrate campaign recaps from Quarto**:
   - Convert `.qmd` recap files to standard `.md` (remove Quarto-specific YAML and formatting)
   - Or keep .qmd files as-is but update `/write-recap` to write plain markdown recaps going forward
   - Remove compiled `.html` output files (the wiki is the canonical rendered view now)

3. **Remove duplicate/unnecessary files**:
   - Identify files that are now fully represented in the codex
   - Remove old calendar JPGs that are replaced by the interactive component (keep Illustrator sources)
   - Clean up `.DS_Store` files
   - Archive old prep sheets and blank grids if no longer used

4. **Update session prep format**:
   - Future session preps can reference codex entries by slug
   - Add a "Codex References" section to prep template listing relevant entry slugs
   - Update the prep-session skill template accordingly

5. **Final INDEX.md verification**:
   - Run `npm run index` one final time
   - Verify all entries, cross-references, and campaign links are correct
   - Spot-check 10-20 entries for accuracy

**Files Created/Modified**:
- Various files in parent TTRPG repo (reorganization)
- `../.claude/skills/write-recap/SKILL.md` — Updated template (if changing recap format)
- `../.claude/skills/prep-session/SKILL.md` — Updated template with codex references
- `INDEX.md` — Final verified version

**Success Criteria**:
- [ ] Repo structure is cleaner and more navigable
- [ ] No duplicate information exists between codex and old files
- [ ] Completed campaign raw files are archived but preserved
- [ ] Illustrator source files for maps and calendar are preserved
- [ ] VTT Product path unchanged
- [ ] Root repo path unchanged
- [ ] All skills work correctly with the new file structure
- [ ] `npm run build` still succeeds for iouns-eye
- [ ] A full end-to-end test: run `/write-recap`, verify extraction, run `/prep-session`, verify codex context

**Known Risks**:
- Reorganization could break file references in Claude Code conversation history (mitigate by not moving active campaign directories)
- Some old files may have sentimental or reference value — don't delete without asking
- Migration from .qmd is optional — Carlos may prefer to keep the format for existing files
